...
# 共通MLPredicatorを使用
try:
    valuation_model = MLPredictor(model_path='.')
    ML_AVAILABLE = True
    logger.info("Using local MLPredictor with ML capabilities.")
...
app = FastAPI(
    title="Real Estate Valuation API (Local)",
...
@app.get("/")
async def root():
    return {"message": "Real Estate Valuation API (Local)", "status": "running", "port": 8001}
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "ml_model": ml_status,
        "service_type": "Local MLPredictor",
        "version": "2.0.0-local"
    }
