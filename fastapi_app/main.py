...
try:
    ...
    ML_AVAILABLE = True
    logger.info("Using integrated lightweight model with ML capabilities.")
except ImportError:
...
async def health_check():
    ...
    ヘルスチェック - MLモデルの状態も含む
    """
    ml_status = "available" if valuation_model.is_model_available() else "unavailable"
    return {
        "status": "healthy",
        "ml_model": ml_status,
        "service_type": "Random Forest/XGBoost only"
    }
...
async def predict_valuation(property_data: PropertyData):
    """
    Random Forest/XGBoost専用査定エンドポイント
    ...
    try:
        ...
        # MLモデル利用可能性の事前チェック
        if not valuation_model.is_model_available():
