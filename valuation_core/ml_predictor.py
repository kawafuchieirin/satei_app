"""
共通MLモデル査定システム (Random Forest / XGBoost)
Django・FastAPI共通で使用
"""
import json
...
class MLPredictor:
    """Random Forest / XGBoost専用査定システム（共通モジュール）"""
    ...
    def _get_default_model_path(self) -> str:
        """デフォルトのモデルパスを取得"""
        # Docker環境では /app/models/, ローカルでは相対パス
    ...
    def _try_load_ml_model(self):
        ...
        try:
            import joblib  # Dynamic import for safety
            import time
            
            start_time = time.time()
            model_path = Path(self.model_path) / 'valuation_model.joblib'
            encoders_path = Path(self.model_path) / 'label_encoders.joblib'
            scaler_path = Path(self.model_path) / 'scaler.joblib'
    ...
    def _ml_predict(self, prefecture: str, city: str, land_area: float,
        ...
        try:
            import pandas as pd  # Dynamic import for safety
            import numpy as np
            ...
            raise RuntimeError(f"Prediction failed due to ML model error: {e}")
